...
# -----------------------------------------------------------------
# 1. settings.py   -> WIDTH, HEIGHT, COLORS, HORIZON_Y
# 2. walker.py     -> Class Walker (Victoria's Body)
# 3. ui_utils.py   -> Drawing helpers (Balloons/Rects)
# 4. engine.py     -> The State Machine (Now with 15s delay)
...
class Walker:
    ...
    def update_animation(self, swing, arm_swing, bob):
        ...
        s = self.scale
        cx, cy = self.pos[0], self.pos[1] + (bob * s)
...
class GameApp:
    def __init__(self, root):
        ...
        self.keys = {}
        self.walk_timer = 0
        self.thought_offset_y1 = 0 
        self.thought_offset_y2 = 0 
        self.setup_world()
    ...
    def get_dance_values(self, t):
        beat = (t * 2) % 1.0
        s = math.sin(t * 12) * 15 if beat > 0.5 else math.cos(t * 8) * 5
        a = math.sin(t * 15) * 25 
        b = abs(math.sin(t * 6)) * -15
        jump = -40 * math.pow(math.sin(t * 2), 10) if t > 1.2 else 0
        return s, a, b + jump
    ...
    def run(self):
        ...
        if self.state == 'INTRO':
            ...
            self.walker.update_animation(*self.get_dance_values(dt))
            if dt > 3: self.trigger_state('SPEAKING')
        elif self.state == 'SPEAKING':
            self.walker.update_animation(0, 0, 0)
            self.draw_speak("I am Victoria. I am 8 years old.\nI am grown-up and I can go wherever I want!")
            if now - self.state_start > 4.5: self.trigger_state('PLAYING')
            ...
            # Thought 1: The realization (Starts at 2.5s)
            if dt > 2.5: self.thought_offset_y1 += 1.2 
            msg1 = "Uhmmm!..." if dt < 2.5 else "I think that I am trapped inside a stupid code!"
            if self.thought_offset_y1 < 580:
                self.draw_think(msg1, self.thought_offset_y1)
            
            # Thought 2: The boredom (Starts 15s after the Crisis began)
            if dt > 17.5: # 2.5 (initial) + 15 (wait) = 17.5 total
                self.thought_offset_y2 += 0.8 # Slightly slower drift
                if self.thought_offset_y2 < 580:
                    self.draw_think("this is boring...", self.thought_offset_y2)
    ...
    def draw_roman_intro(self):
        tit, sub = "V I C T O R I A ' S   W A L K", "T H E   R E T U R N"
    ...
    def draw_speak(self, text):
        ...
        off_y = 65 * self.walker.scale
        t_id = self.canvas.create_text(cx, cy - off_y, text=text, fill="black", font=("Arial", 11, "bold"), justify="center", tags="ui")
    ...
    def draw_think(self, text, drift_y=0):
        cx, cy = self.walker.pos
        s = self.walker.scale
        current_y = cy - (65 * s) - drift_y
        t_id = self.canvas.create_text(cx, current_y, text=text, fill="black", font=("Arial", 11, "italic", "bold"), tags="ui")
        b = self.canvas.bbox(t_id)
