...
class FibonacciCanvasApp:
    def __init__(self, root):
        self.root = root
        self.root.title("VENTENVS INFINITE-PHI")
        ...
        self.canvas_h = 500
        self.phi = 1.618
        self.current_bg_rgb = (100, 100, 100)
        ...
        # Slider (Controls the size of the NEXT batch of shapes)
        ...
        self.btn_new_gen = tk.Button(
            self.btn_frame, text="NEW GENERATION", command=self.generate_new_batch,
            bg=self.btn_dark, fg=self.text_light, width=15,
            activebackground=self.accent, font=("Consolas", 10, "bold"), relief="flat"
        )
        self.btn_new_gen.grid(row=0, column=0, padx=5)
        ...
        # Optional: Clear Button to actually empty the canvas
        self.btn_clear = tk.Button(
            root, text="CLEAR CANVAS", command=self.clear_canvas,
            bg="#441111", fg="white", font=("Consolas", 8), relief="flat"
        )
    ...
    def generate_new_batch(self):
        """Adds 3 new shapes to the existing canvas and updates background DNA."""
        ...
        # Add 3 shapes on top of whatever is already there
        self.draw_shape(base_large)
    ...
    def add_extra_shape(self):
        """Adds a single recursive Fibonacci shape on top."""
        self.last_radius = self.last_radius / self.phi
        if self.last_radius < 5: self.last_radius = 40 
        self.draw_shape(self.last_radius)

    def clear_canvas(self):
        """Explicitly wipes the canvas clean."""
